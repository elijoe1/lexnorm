...
def evaluate(raw, gold, pred, ignCaps=False, verbose=False):
    ...
    if len(gold) != len(pred):
        err(
            "Error: gold normalization contains a different numer of sentences("
            + str(len(gold))
    ...
    for sentRaw, sentGold, sentPred in zip(raw, gold, pred):
        if len(sentGold) != len(sentPred):
            err(
                "Error: a sentence has a different length in you output, check the order of the sentences"
            )
...
def loadNormData(path):
    ...
    for line in open(path):
        ...
        if tok == [""] or tok == []:
            ...
            if len(tok) > 2:
                err(
                    "erroneous input, line:\n"
                    + line
                    + "\n in file "
                    + path
                    + " contains more then two elements"
                )
            if len(tok) == 1:
                tok.append("")
            curSent.append(tok)
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--ignCaps", action="store_true", default=False, help="lowercase everything"
    )
    parser.add_option(
        "--verbose",
        action="store_true",
        default=False,
        help="print the errors made by the system",
    )
