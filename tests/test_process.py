...
def test_process_data_file():
    ...
    cands = load_candidates(os.path.join(DATA_PATH, "hpc/fixed_dev.norm")).fillna(0)
    ...
    for i in range(50):
        ...
        assert features[cand]["orig_norms_seen"] == (
            normalisations[raw].get(raw, 0) if raw in normalisations else 0
        )
        assert features[cand]["orig_frac_norms_seen"] == pytest.approx(
            normalisations[raw].get(raw, 0)
            / sum([v for k, v in normalisations[raw].items()])
            if raw in normalisations
            else 0
        )
        assert features[cand]["orig_length"] == len(raw)
        assert features[cand]["orig_in_feature_lex"] == (raw in feature_lex)
